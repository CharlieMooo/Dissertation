...
globals [
  ...
  temp-cost-college-collect
  temp-convenience-college-collect
  temp-time-college-collect
  temp-price-convenience-time-college-collect
  temp-prior?
  temp-p-ratio
  temp-price
  ...
  p-post-large
  p-mover-xlarge
  p-college-collect-letter
  p-college-collect-small
  p-college-collect-medium
...
;; Set The Global Variables
  ...
  set p-college-collect-letter 0.5 * p-post-letter
  set p-college-collect-small 0.5 * p-post-small
  set p-college-collect-medium 0.5 * p-post-medium
...
to compare-items
  ...
  if any? items-s with [type-s = temp-type-d and volume-s = temp-volume-d and original-p = temp-average-orininal-p] [ask n-of temp-sellers items-s with [type-s = temp-type-d and volume-s = temp-volume-d and original-p = temp-average-orininal-p] [
...
to confirm-highest-utility
  (ifelse
    ...
    ; 1. volume 1-3, distance <= 10, walk, postage, uni-collect
    self-collection? = true and walk? = true and public-transport? = false and postage? = true and temp-prior? = false [
      ...
      (ifelse
        volume-s = 1 [set temp-cost-college-collect p-college-collect-letter]
        volume-s = 2 [set temp-cost-college-collect p-college-collect-small]
        volume-s = 3 [set temp-cost-college-collect p-college-collect-medium])
      set temp-convenience-college-collect 8
      set temp-time-college-collect 6
      set temp-price-convenience-time-college-collect (1 - (price + temp-cost-college-collect) / original-p) * temp-co-price * 10 + temp-convenience-college-collect * temp-co-convenience + temp-time-college-collect * temp-co-time

      ifelse
      temp-price-convenience-time-walk < temp-price-convenience-time-postage [
        ifelse temp-price-convenience-time-postage > temp-price-convenience-time-college-collect [
          set utility-price-convenience-time temp-price-convenience-time-postage
          set delivery-method 3
        ] [
          set utility-price-convenience-time temp-price-convenience-time-college-collect
          set delivery-method 4
        ]
      ] [
        ifelse temp-price-convenience-time-walk > temp-price-convenience-time-college-collect [
    ...
    ; 2. volume 1-3, distance <= 10, walk, prior postage, uni-collect
    self-collection? = true and walk? = true and public-transport? = false and postage? = true and temp-prior? = true [
      ...
      set temp-convenience-postage 8
      set temp-time-postage 8
      set temp-price-convenience-time-postage (1 - (price + temp-cost-postage) / original-p) * temp-co-price * 10 + temp-convenience-postage * temp-co-convenience + temp-time-postage * temp-co-time

      (ifelse
        volume-s = 1 [set temp-cost-college-collect p-college-collect-letter * (1 + prior-post-extra-rate)]
        volume-s = 2 [set temp-cost-college-collect p-college-collect-small * (1 + prior-post-extra-rate)]
        volume-s = 3 [set temp-cost-college-collect p-college-collect-medium * (1 + prior-post-extra-rate)])
      set temp-convenience-college-collect 10
      set temp-time-college-collect 8
      set temp-price-convenience-time-college-collect (1 - (price + temp-cost-college-collect) / original-p) * temp-co-price * 10 + temp-convenience-college-collect * temp-co-convenience + temp-time-college-collect * temp-co-time
    ...
    ; 4. volume 1-3, distance > 10, public, postage, uni-collect
    self-collection? = true and walk? = false and public-transport? = true and postage? = true and temp-prior? = false [
      ...
      temp-price-convenience-time-public-transport < temp-price-convenience-time-postage [
        ...
        ifelse temp-price-convenience-time-public-transport > temp-price-convenience-time-college-collect [
    ...
    ; 5. volume 1-3, distance > 10, public, prior postage, uni-collect
    ...
    ; 7. volume 1-3, distance > 10, accessibility < 8, postage, uni-collect
    ...
    ; 8. volume 1-3, distance > 10, accessibility < 8, prior postage, uni-collect
...
Based on the basic model, this model added the function that items can be collectively delivered to the university and collected there, which will participate in the comparison with other delivery methods. This new delivery method is equivalent to setting click and collect point in the universities. Referring to Yuen, Wang, Ng and Wong (2018), click and collect point will reduce more than 50% of the delivery cost. Therefore, this method can be described as: extra cost depends on the size, half price of the postage service in same level, less high convenience ignoring the distance, two-day delivery, applicable to smaller items that are more than 2 days from the maximum tolerant days, prior one-day postage service available for items that are more than 1 day from the maximum tolerant days and extra prior postage cost applied.
...
Yuen, K., Wang, X., Ng, L. and Wong, Y., 2018. An investigation of customers’ intention to use self-collection services for last-mile delivery. Transport Policy, 66, pp.1-8.
